...
const startGame = () => {
  ...
  for (let cell of cells) {
    ...
    cell.style.background = "##2ef37065";
  }
};
...
const gameOver = (gameWon) => {
  for (let index of winCombos[gameWon.index]) {
    ...
    document.getElementById(index).style.backgroundColor = "#52ee86";
  }
  ...
  declareWinner(
    gameWon.player === user ? "¡Has ganado!" : "¡La IA ha ganado!"
  );
};
...
const checkTie = () => {
  if (emptySquares().length === 0) {
    ...
    for (let cell of cells) {
      cell.style.backgroundColor = "#52ee86";
      cell.removeEventListener("click", handleClick, false);
    }
